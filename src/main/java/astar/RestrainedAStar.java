...
public class RestrainedAStar {
    /*
     * get the restrainedAStar shortest path from start to target
     */
    public static Path singlePath(Graph<RoadNode, RoadEdge> g, RoadNode start, RoadNode target, RoadNode coreNode) {
        ...
        long estimatedWeight = (long) (MillerCoordinate.distance(start, target) * 0.85);
        ...
        while(!priorityQueue.isEmpty()) {
            ...
            updateForwardPriorityQueue(g, infoNodes, priorityQueue, inode, target, coreNode);
        }
        return path;
    }

    /*
     * get the time-dependent restrainedAStar shortest path from start to target
     */
    public static Path timeDependentSinglePath(Graph<RoadNode, RoadEdge> g, long time, RoadNode start, RoadNode target, RoadNode coreNode) {
        long starttime = time;
        Map<String, AStarInfoNode> infoNodes=new HashMap<String,AStarInfoNode>();  //为了根据roadNode找到infoNode
        LinkedList<AStarInfoNode> priorityQueue = new LinkedList<AStarInfoNode>();
        ...
        while(!priorityQueue.isEmpty()) {
            ...
            timeDependentUpdateForwardPriorityQueue(g, infoNodes, priorityQueue, inode, target, coreNode);
        }
        return path;
    }

    public static void updateForwardPriorityQueue(Graph<RoadNode, RoadEdge> g, Map<String, AStarInfoNode> infoNodes, LinkedList<AStarInfoNode> priorityQueue, AStarInfoNode inode, RoadNode target, RoadNode coreNode) {
        ...
        while(it.hasNext()) {
            ...
            if(nextNode.getBelongTo_incoming()!=coreNode && nextNode.getBelongTo()!= coreNode){
                continue;
            }
            if(infoNodes.containsKey(nextNode.getOsmId())){
                ...
                weightFromParent = nextEdge.getLength();
                long estimatedWeight = (long) (MillerCoordinate.distance(nextNode, target)*0.85);
    ...
    public static void timeDependentUpdateForwardPriorityQueue(Graph<RoadNode, RoadEdge> g, Map<String, AStarInfoNode> infoNodes, LinkedList<AStarInfoNode> priorityQueue, AStarInfoNode inode, RoadNode target, RoadNode coreNode) {
        //get the all connected edge of roadNode
        Set<RoadEdge> edgeSet = g.outgoingEdgesOf(inode.getRoadNode());
        Iterator<RoadEdge> it = edgeSet.iterator();
        while(it.hasNext()) {
